...
import Project, { FileTextChanges, FormatCodeSettings, LanguageService, SourceFile, ts, UserPreferences } from 'ts-simple-ast';
...
export function removeAllUnused(project: Project, f: SourceFile, formatOptions: FormatCodeSettings = {}, preferences: UserPreferences = {}) {
  const d = getAllUnusedCodeFixes(project.getLanguageService(), f,formatOptions , preferences)
  applyCodeFixes(project, f, d)
}

export function getAllUnusedCodeFixes(languageService: LanguageService, sourceFile: SourceFile, formatOptions: FormatCodeSettings = {}, preferences: UserPreferences = {}): CodeFixActionExtended[]{
  return getAllSupportedCodeFixes(languageService, sourceFile, 0, sourceFile.getText().length, unusedCodes, formatOptions, preferences)
  }
...
export function getUnusedCodeFixes(languageService: LanguageService, sourceFile: SourceFile, pos: number = 0, end: number = sourceFile.getText().length - 1,  formatOptions: FormatCodeSettings = {}, preferences: UserPreferences = {}): CodeFixActionExtended[]{
return getAllSupportedCodeFixes(languageService, sourceFile, pos, end, unusedCodes, formatOptions, preferences)
}

export function getAllSupportedCodeFixes(languageService: LanguageService, sourceFile: SourceFile, pos: number = 0, end: number = sourceFile.getText().length - 1, errorCodes?: number[], formatOptions: FormatCodeSettings = {}, preferences: UserPreferences = {}): CodeFixActionExtended[] {
  const filePtah = sourceFile.getFilePath()
  const all = (errorCodes ? getAllSupportedCodeFixeDefinitions().filter(e => errorCodes.includes(e.code)) : getAllSupportedCodeFixeDefinitions())
    .map(c => {
      try {
        return languageService.getCodeFixesAtPosition(filePtah, pos, end, [c.code], formatOptions, preferences)
          .map(tsc => ({ ...tsc.compilerObject, diagnostic: c, changes: tsc.getChanges() }));
      } catch (error) {

      }
    })
  return flat(all.filter(notUndefined))
    .filter(c => c.changes.find(c => c.getFilePath() === filePtah) && (c.fixName || c.fixAllDescription))
}
...
function getAllCodeFixesErrorCodes() {
  return getTsDiagnostics().map(d => d.code).filter(d => d)
}