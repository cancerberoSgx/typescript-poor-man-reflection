...
import { ExtractorFn, Extractor, ExtractorClass } from './types'
...
export const defaultExtractors: { [k: string]: Extractor } = {
  TypeText: (n: CallExpression, index, getter, options, fileVariableAccessor) => {
    return options.extractorDataMode==='asStringLiteral' ?  quote(n.getTypeArguments()[0].getText()) : {
  ...
  NodeText: (n: CallExpression, index, getter, options, fileVariableAccessor) => {
    ...
    return options.extractorDataMode==='asStringLiteral' ?  (c ? `${JSON.stringify(c.getText())}` : '') : {
      argument: getter(index),
  ...
  BodyText: (n: CallExpression, index, getter, options, fileVariableAccessor) => {
    ...
    return  options.extractorDataMode==='asStringLiteral' ?  (TypeGuards.isBodyableNode(f) ? `${JSON.stringify(f.getBodyText())}` : '') : {
      argument: getter(index),
  ...
  ThisBlockText: (n: CallExpression, index, getter, options, fileVariableAccessor) => {
    ...
    return  options.extractorDataMode==='asStringLiteral' ?  (result) : {
      argument: getter(index),
