 ...
 */
export function extractorGetterBuilder(options: ReplaceFileFunctionCallOptions & { extractorDataFolderFileName: string, filePattern?: string }, index: number, sourceFile: SourceFile, c: CallExpression): ExtractorGetter {
...
export function writeExtractorData(sourceFile: SourceFile, options: {
...
} = { extractorDataFolderFileName: '__tsd_check_runtime__', clean: false, extractorDataVariableName: '__extractor_prepend__' }, callExpressions: CallExpression[], prependToFile: string[]) {
  ...
  if (!options.extractorDataMode || options.extractorDataMode === 'prependVariable') {
    ...
    if (!options.clean) {
      const { dataFile, fileId } = ensureDataFile(sourceFile, { ...options, extractorDataFolderFileName: options.extractorDataFolderFileName! }, prependToFile)
      dataFile.saveSync();
    }
...
function ensureDataFile(sourceFile: SourceFile, options: { extractorDataVariableName: string; clean: boolean; extractorDataMode?: "prependVariable" | "folderFile" | undefined; extractorDataFolderFileName: string }, prependToFile: string[]) {
...
function getFileId(sourceFile: SourceFile, { extractorDataFolderFileName }: { extractorDataFolderFileName: string }) {
  ...
  const sorted = d.getSourceFiles().filter(f => includeFile(f, { extractorDataFolderFileName })).map(f => f.getBaseName()).sort()
